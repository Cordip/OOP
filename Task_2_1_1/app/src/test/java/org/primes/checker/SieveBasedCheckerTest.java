...
class SieveBasedCheckerTest {

    private SieveBasedChecker checker;

    @BeforeEach
    void setUp() {
        checker = new SieveBasedChecker();
    }
    ...
    void testContainsNonPrime_ContainsZero() {
     ...
     @Test
     void testContainsNonPrime_AllNegativeOrZeroOrOne() {
          int[] arr = {-1, -5, 0, 1, -100};
          assertTrue(checker.containsNonPrime(arr));
     }

     @Test
     void testContainsNonPrime_OnlyTwos() {
          int[] arr = {2, 2, 2};
          assertFalse(checker.containsNonPrime(arr));
     }
    ...
    @Test
    @Timeout(value = 30, unit = TimeUnit.SECONDS) // Sieve generation can be slow
    void testContainsNonPrime_LargeArrayAllPrimes() {
         ...
         int size = 5000;
         int maxVal = 5_000_000;
         int[] arr = generatePrimesArray(size, 1_000_000, maxVal);
        assertFalse(checker.containsNonPrime(arr));
    }
    ...
    void testContainsNonPrime_LargeArrayOneNonPrimeAtEnd() {
     ...
     @Test
     void testContainsNonPrime_NumberLargerThanSieveLimit() {
         // Relies on the correctness of Arrays.stream().max().
         // The internal check `if (number > sieveLimit)` is a safeguard.
     }

     @Test
    void testContainsNonPrime_MaxValLessThanTwo() {
        int[] arr1 = {0, 1, -5};
        assertTrue(checker.containsNonPrime(arr1));

        int[] arr2 = {1};
        assertTrue(checker.containsNonPrime(arr2));

         int[] arr3 = {-10, -2};
         assertTrue(checker.containsNonPrime(arr3));
    }
}